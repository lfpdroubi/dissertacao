...
```{r}
nhpidata <- Quandl("YALE/NHPI", type="ts", start_date="1990-01-01")
rhpidata <- Quandl("YALE/RHPI", type="ts")
```
...
ggplot(nhpidata, as.numeric = FALSE) +
  geom_line() +
   labs(
...
`r brf(window(nhpidata, start = 2006 + 6/12, end = 2006 + 6/12)[[1]])` pontos,
entrando então em tendência de baixa, até atingir os `r brf(window(nhpidata, start = 2012 + 0/12, end = 2012 + 0/12)[[1]])`, em janeiro de 2012, quando se iniciou nova tendência de 
alta, que perdura até os dias atuais. No momento em que se escreve esta 
dissertação o índice se encontra com `r brf(last(nhpidata))` pontos, maior valor
da série histórica.
...
ggplot(window(rhpidata, start = 1990 + 0/12), as.numeric = FALSE) +
  geom_line() +
   labs(
...
Na atualidade este índice se encontra em `r brf(last(rhpidata))` pontos, sendo 
que o maior valor da série histórica se deu em dezembro de 2005.

Em janeiro de 1997, o índice estava em `r brf(window(rhpidata, start = 1997 + 0/12, end = 1997 + 0/12)[[1]])` pontos. Desta data em diante, o índice entrou em franca tendência 
de alta até atingir os
`r brf(window(rhpidata, start = 2005 + 11/12, end = 2005 + 11/12)[[1]])` pontos, 
...
ggplot(window(rhpidata, start = 1953 + 0/12), as.numeric = FALSE) +
  geom_line() +
   labs(
     title = "Case-Shiller U.S. National Real Home Price Index", 
     subtitle = "Not Seasonally Adjusted | Frequency: Monthly", 
...
`r month.abb[(time(rhpidata)[which.min(rhpidata)] %% 1)*12+1]`/`r floor(time(rhpidata)[which.min(rhpidata)])`, quando o índice atingiu a marca
de `r brf(rhpidata[which.min(rhpidata)])` pontos.

É interessante notar que, em um século, desde 1890, o índice de preços reais de 
imóveis atingiu um valor máximo de apenas 
`r brf(window(rhpidata, start = 1989 + 7/12, end = 1989 + 7/12)[[1]])` pontos, o
...
`r brf(last(rhpidata) - window(rhpidata, start = 1989 + 7/12, end = 1989 + 7/12)[[1]])`
pontos percentuais acima daquela marca e a apenas 
`r brf(window(rhpidata, start = 2005 + 11/12, end = 2005 + 11/12)[[1]] - last(rhpidata))`
...
`r brf(window(rhpidata, start = 2012 + 0/12, end = 2012 + 0/12)[[1]])` pontos,
apenas `r brf(window(rhpidata, start = 1989 + 7/12, end = 1989 + 7/12)[[1]] - window(rhpidata, start = 2012 + 0/12, end = 2012 + 0/12)[[1]])` pontos abaixo do topo histórico 
...
muito além do escopo deste trabalho. Uma boa discussão pode ser vista em @bresser2018.
...
longa maturidade, ou seja, de 30 anos, está em `r brf(last(df)$rate)`% a.a. Para
os títulos com vencimento em 20 anos, a taxa está em 
`r brf(last(df[which(df$bond == 'GS20'), ])$rate)`% a.a. e para os títulos com
vencimento em 10 anos está em 
`r brf(last(df[which(df$bond == 'GS10'), ])$rate)`% a.a., todos nos seus valores 
mínimos da série histórica.



```{r fred, out.width="\\textwidth", fig.cap="Taxas dos \\emph{treasuries bonds} desde abril/1953."}
...
*underperform* ou *sell*), assim como o método de Black&Scholes permite o
