...
Segundo @shelter [p. 188-189], ...

```{r, fig.cap = "Diagrama esquemático do funcionamento do Mercado Imobiliário."}
grViz("
      digraph nicegraph {
  ...
  'Mercado Imobiliário'             [width = 3]
  'Mercado Imobiliário' -> 'Mercado de Insumos'
  'Mercado Imobiliário' -> 'Processo de Produção'
  'Mercado Imobiliário' -> 'Mercado de Residências'
  'Mercado de Insumos' -> 'Terra'            [lhead = cluster0]
  'Processo de Produção' -> 'Construtores'   [lhead = cluster1]
  'Mercado de Residências' -> 'Compradores'  [lhead = cluster2]
...
O detalhamento destes modelos dinâmicos está fora do escopo deste trabalho. No
entanto, é possível através de modelos mais simples de oferta e demanda entender
o efeito da inelasticidade da oferta, como pode ser visto na Figura 
\@ref(fig:choque): um aumento na demanda pode ser modelado como um deslocamento
da curva da demanda para a direita, levando os preços de equilíbrio inicial 
$P_0$ ao novo preço de equilíbrio $P_1$. Com o aumento da oferta, no longo prazo,
o preço de equilíbrio retorna ao ponto $P_2$.

```{r choque, fig.cap="Efeitos de um choque de demanda num mercado com oferta inelástica."}
knitr::include_graphics("images/choque_demanda_oferta_inelastica.png")
```
\bcenter
Fonte: @regulation [p. 10]
\ecenter

Num mercado com uma oferta menos inelástica, a volatilidade é muito menor, o que
pode ser visto na figura \@ref(fig:choque2): o deslocamento da curva de oferta
provoca um aumento mais rápido da oferta, fazendo com que o preço inicial de 
equilíbrio $P_0$ aumente muito menos significativamente ($P_1$) do que no caso
anterior, voltando a um preço de equilíbrio no longo prazo ($P_2$) praticamente 
igual ao preço inicial.


```{r choque2, fig.cap="Efeitos de um choque de demanda num mercado com oferta elástica."}
knitr::include_graphics("images/choque_demanda_oferta_elastica.png")
```
\bcenter
Fonte: @regulation [p. 10]
\ecenter
...
ocorrer ainda o problema de uma resposta mal desenhada pelo governo ao aumento 
de preços inicial. Algumas medidas podem funcionar no médio prazo, aumentando a 
oferta temporariamente e evitando uma grande alta dos preços, porém, no longo
prazo, um posterior novo choque de demanda pode vir a elever os preços a 
níveis muito superiores, como ilustrado na Figura \@ref(fig:choque3). 

```{r choque3, fig.cap="Efeitos de uma resposta governamental inadequada a um choque de demanda."}
knitr::include_graphics("images/choque_demanda_resposta_inadequada.png")
```
\bcenter
...
ou não, no entanto o modo deconstrução destes índices de preços difere de país 
