...
dgs30 <- fredr(series_id = "DGS30",
              observation_start = as.Date("1962-01-02"))
dados <- left_join(dgs10, dgs20, by = "date", suffix = c(".DGS10", ".DGS20"))
dados <- left_join(dados, dgs30, by = "date")
dados <- dados[, c("date", "value.DGS10", "value.DGS20", "value")]
colnames(dados) <- c("date", "DGS10", "DGS20", "DGS30")
dados <- dados %>% gather("bond", "rate", -date)
saveRDS(dados, "data/FRED_daily")
```
...
```{r, eval = FALSE}
rint <- fredr(series_id = "DFII10")
saveRDS(rint, file = "data/rint")
```


```{r realinterests, fig.cap="Taxas de juros reais. Títulos com vencimento de 10 anos protegidos contra inflação."}
rint <- readRDS(file = "data/rint")
ggplot(data = rint, aes(x = date, y = value)) +
  geom_line(color = 'blue') +  
  xlab("Ano") +
  ylab("Taxa real(a.a.)") +
  #scale_x_continuous(expand = c(0, 0)) +
  ...
  geom_rect(data=recessions.df[which(recessions.df$Peak >= as.Date("2003-01-02")), ], 
            aes(xmin=Peak, xmax=Trough, ymin=-Inf, ymax=+Inf),
            inherit.aes=F, fill='grey', alpha=0.5)
```
