...
a. *Home Price Index* de Case-Shiller
...
a necessidade de comparação da evolução do índice de preços com outras variáveis macroeconômicas, *i.e.*  com a simples análise do comportamento do índice e (c)
...
```{r whasington, fig.cap="Preços da terra divididos em quantis ao longo de um ciclo do \\gls{MI}."}
...
```{r whasington2, fig.cap="Inflação dos preços dos imóveis divididos em componentes."}
...
```{r whasington3, fig.cap="Desinflação dos preços dos imóveis divididos em componentes."}
...
RHVRUSQ156N$date <- seq(ISOdate(1956,1,1), ISOdate(2020,10,1), by = "quarters")
RRVRUSQ156N$date <- seq(ISOdate(1956,1,1), ISOdate(2020,10,1), by = "quarters")
```
