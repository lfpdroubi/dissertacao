...
A elasticidade-preço da demanda, \(\varepsilon_{pd}\), definida conforme a
equação \eqref{eq:epdemanda} \autocite[p.~302]{varian}, mede a quantidade marginal
\(\mathrm{d} q_d\) que será demandada do mercado se o preço \(p\) de uma mercadoria
aumenta ou diminui em uma unidade de preço. Geralmente a elasticidade-preço da
oferta é, então, um número negativo \autocite[289]{varian}.
\begin{equation}
\varepsilon_{pd} = \frac{p}{q_d}\frac{\mathrm{d} q_d}{\mathrm{d} p}
\label{eq:epdemanda}
\end{equation}
...
natural das variáveis \autocite[307-308]{varian}:
\begin{equation}
\varepsilon_pd = \frac{\mathrm{d} \ln(q_d)}{\mathrm{d} \ln(p)}
...
A elasticidade-renda da demanda, \(\varepsilon_{yd}\), mede como muda a
quantidade demandada \(q_d\) quando a renda \(y\) do consumidor varia \autocite[300]{varian}.
De acordo com a elasticidade-renda os bens podem ser classificados em bens
normais, \emph{i.e.} bens cuja demanda aumenta com o aumento da renda; bens
inferiores, \emph{i.e.} bens cuja demanda diminui com o aumento da renda; e bens de
luxo (\(\varepsilon_r > 1\)), \emph{i.e.} bens cujo aumento na demanda é mais do que proporcional ao aumento da renda (um aumento de 1\% na renda conduz a um aumento
de mais de 1\% da demanda).

A elasticidade-renda da demanda é calculada conforme a equação
\eqref{eq:erdemanda} e sua análise pode ser feita a partir das curvas de Engel,
como na Figura \ref{fig:engel1}.
\begin{equation}
\varepsilon_{yd} = \frac{y}{q_d}\frac{\mathrm{d} q_d}{\mathrm{d} y}
\label{eq:erdemanda}
\end{equation}
\begin{figure}[H]
...
mercado de bens de luxo (\(\varepsilon_{yd} > 1\)), com o aumento da renda as
pessoas podem ficar mais \emph{dispostas a comprometer} uma parte um pouco maior do
orçamento neste mercado, enquanto em um mercado de bens normal
(\(\varepsilon_{yd} < 1\)), as pessoas podem estar \emph{obrigadas a comprometer} boa
parte do seu orçamento neste mercado, mas, à medida que a renda aumenta, a
proporção da renda aplicada neste mercado diminui.
\begin{figure}[H]
...
\paragraph{Elasticidade-preço da oferta}\label{elasticidade-preuxe7o-da-oferta}}

Analogamente à elasticidade-preço da demanda, a elasticidade-preço da oferta
\(\varepsilon_{ps}\) mede a variação na quantidade ofertada \(q_s\) de uma
mercadoria quando os preços \(p\) variam em uma unidade, sendo portanto, em geral,
um número positivo (quando os preços aumentam, a quantidade ofertada aumenta).
\begin{equation}
\varepsilon_{ps} = \frac{p}{q_s}\frac{\mathrm{d} q_s}{\mathrm{d} p}
\label{eq:epoferta}
\end{equation}
...
longa maturidade, ou seja, de 30 anos, está em 1,87\% a.a.

Para os títulos com vencimento em 20 anos, a taxa está em 1,80
\% a.a. e para os títulos com vencimento em 10 anos está em 1,31\% a.a.,
...
Ainda mais impressionante é a análise das taxas juros reais, o que pode ser
visto na Figura \ref{fig:realinterests}.
\begin{figure}[H]

{\centering \includegraphics[width=0.7\linewidth]{images/realinterests-1} 

}

\caption{Taxas de juros reais. Títulos com vencimento de 10 anos protegidos contra inflação.}\label{fig:realinterests}
\end{figure}
...
\caption{HPI, Custos de Construção, População e Taxa de Juros nos EUA, desde 1890.}\label{fig:unnamed-chunk-21}
\end{figure}
\bcenter
...
\caption{Índices de Preços de imóveis Residenciais e Comerciais no EUA.}\label{fig:unnamed-chunk-22}
\end{figure}
\bcenter
