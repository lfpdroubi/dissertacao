...
pacote \pkg{strucchange} [@strucchange]. As linhas tracejadas em vermelho 
representam estes pontos de corte.

```{r, eval = FALSE}
nhpi <- Quandl("YALE/NHPI", type="ts", start_date="1975-01-01")
rhpiAnual <- Quandl("YALE/RHPI", type="ts", collapse = "annual")
saveRDS(nhpi, file = "data/nhpi")
saveRDS(rhpiAnual, file = "data/rhpiAnual")
```

```{r}
nhpi <- readRDS("data/nhpi")
rhpiAnual <- readRDS("data/rhpiAnual")
```
...
autoplot(breakpoints(rhpiAnual ~ 1), cpt.colour = 'red', cpt.linetype = 'dashed') +
  labs(x = "Ano", y = "Índice (1890 = 100)")
```
\bcenter
\small Fonte -- O autor, à partir de dados de @QuandlWIKI.
...
rhpiDF <- data.frame(date = as.Date(as.yearmon(time(rhpi))),
                     Y = as.matrix(rhpi))
#ggplot(window(rhpi, start = 1990 + 0/12), as.numeric = FALSE) +
ggplot(rhpiDF[rhpiDF$date >= '1990-01-01', ], aes(x = date, y = Y)) +
  geom_line() +
   labs(
  ...
  geom_rect(data=recessions.trim, 
            aes(xmin=as.Date(Peak), xmax=as.Date(Trough), 
                ymin=-Inf, ymax=+Inf),
          inherit.aes=F,
          fill='grey', alpha=0.5)
```
...
#ggplot(window(rhpi, start = 1953 + 0/12), as.numeric = TRUE) +
#autoplot(window(rhpi, start = 1953)) +
ggplot(rhpiDF[rhpiDF$date >= '1953-01-01', ], aes(x = date, y = Y)) +
   geom_line() +
   labs(
     title = "Índice de Case-Shiller (EUA) HPI Nacional Real",
     subtitle = "Não ajustado sazonalmente | Frequência: Mensal",
